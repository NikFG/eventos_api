...
class Senha implements Rule
    ...
    public function passes($attribute, $value) {
        ...
        if ($this->mixedCase && !preg_match('/(\p{Ll}+.*\p{Lu})|(\p{Lu}+.*\p{Ll})/u', $value)) {
            $this->fail('A :attribute precisa conter pelo menos uma letra maiúscula e uma minúscula.');
        }

        if ($this->letters && !preg_match('/\pL/u', $value)) {
            $this->fail('A :attribute precisa conter pelo menos uma letra.');
        }

        if ($this->symbols && !preg_match('/\p{Z}|\p{S}|\p{P}/u', $value)) {
            $this->fail('A :attribute precisa conter pelo menos um símbolo especial.');
        }
